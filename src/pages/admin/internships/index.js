...
const AdminInternshipsPage = () => {
  ...
  const fetchInternships = async () => {
    ...
    try {
      ...
      // The API returns: { internships: [...], pagination: {...} }
      if (data.internships) {
        setInternships(data.internships);
        setTotalPages(data.pagination.pages);
        setTotalInternships(data.pagination.total);
      } else {
  ...
  const handleDeleteInternship = async (internshipId) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success(`${internship.title} has been deleted successfully`, {
          duration: 4000,
          icon: '🗑️',
          style: {
            background: '#10B981',
  ...
  return (
    <AdminLayout>
      <Head>
        <title>Internship Management | Arsa Nexus Admin</title>
        <meta name="description" content="Manage internship opportunities and applications" />
      </Head>
        ...
        >
          <div>
            ...
            <p className="text-gray-300">
              Manage internship opportunities. Total: <span className="font-semibold text-purple-400">{totalInternships}</span> internships
            </p>
          </div>
          <motion.div
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    type="text"
                    className="w-full pl-10 pr-4 py-3 bg-white/10 border border-white/20 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-white placeholder-gray-400 transition-all duration-200"
              ...
              <div>
                ...
                <select
                  className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-white transition-all duration-200"
              ...
              <div className="flex items-center gap-4">
                <div className="text-center bg-white/5 rounded-xl p-3">
                  <div className="text-2xl font-bold text-purple-400">{totalInternships}</div>
                  <div className="text-sm text-gray-300">Total</div>
                </div>
                <div className="text-center bg-white/5 rounded-xl p-3">
                  <div className="text-2xl font-bold text-green-400">{internships.filter(i => i.status === 'Active').length}</div>
          ...
          {internships.length > 0 ? (
                        ...
                        <div className="p-6 border-b border-white/10">
                          <div className="flex items-start justify-between mb-4">
                            <div className="flex-1">
                              ...
                              <p className="text-gray-300 mt-1">{internship.company}</p>
                            </div>
                          ...
                          <div className="flex items-center gap-4 mb-4 flex-wrap">
                            ...
                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border ${getStatusColorDark(internship.status)}`}>
                              {internship.status}
                            </span>
                          </div>
                        ...
                        <div className="p-6">
                          <div className="grid grid-cols-2 gap-4 mb-6">
                            <div>
                              ...
                              <div className="text-white font-medium">{internship.category}</div>
                            </div>
                            <div>
                          ...
                          <div className="flex items-center justify-between text-sm text-gray-300 mb-6">
                            <div>
                              Stipend: <span className="font-medium text-white">
                                {internship.stipend?.period === 'Unpaid' ? 'Unpaid' : `$${internship.stipend?.amount || 0}/${internship.stipend?.period || 'Month'}`}
                              </span>
                            </div>
                            <div>
                              {daysRemaining > 0 ? (
                          ...
                          <ActionButtons
                            ...
                            deleteApi="/api/internships"
                            onDelete={handleDeleteInternship}
                            viewTitle="View Internship"
                            editTitle="Edit Internship"
                            deleteTitle="Delete Internship"
                            deleteConfirmText={`Are you sure you want to delete "${internship.title}"? This will permanently remove the internship and all associated data.`}
            ...
            >
              ...
              {!searchTerm && !categoryFilter && !statusFilter && (
                <Link
                  href="/admin/internships/new"
                  className="inline-flex items-center gap-2 px-4 py-2 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                  </svg>
  ...
  // Helper function for dark theme status colors
